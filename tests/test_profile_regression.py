...
PRE_REFRESH_CAPABILITY_CHAIN = "Code generation, debugging, optimization"
CANONICAL_INTRO_PARAGRAPH = (
    "Monster Gaming is an AI platform that sits above the engine. "
    "We route your queries through 145+ specialist agents — shader, animation, netcode, "
    "level design, QA, and more — purpose-built for [Unreal Engine](https://monstergaming.ai), "
    "Unity, Godot, and bespoke engines."
)
CANONICAL_MONSTER_GPT_OFFERING = (
    "- **[Monster-GPT](https://monstergaming.ai)** — flagship model that auto-routes to "
    "specialist agents across 30+ game dev disciplines"
)
CANONICAL_OPENAI_API_OFFERING = (
    "- **[OpenAI-compatible API](https://monstergaming.ai/quickstart)** — drop-in replacement "
    "for OpenAI SDKs with game-dev-tuned models"
)
CANONICAL_ENGINE_AWARE_OFFERING = (
    "- **[Engine-aware code generation](https://monstergaming.ai)** — writes gameplay systems, "
    "shaders, networking, and UI that actually compile"
)
CANONICAL_LOKI_CODE_OFFERING = (
    "- **[Loki Code](https://blog.monstergaming.ai/we-built-our-own-ai-coding-cli-in-c-because-ours-got-revoked/)** "
    "— our open-source AI coding CLI, built in C11, 195 tests, 8.1 MB"
)
CANONICAL_NEWSLETTER_LINE = (
...
def test_profile_refresh_loki_specs_stay_out_of_sdk_section(profile_text):
    """4618966 added Loki technical claims to offerings; they must not drift into SDK docs."""
    sdk_section = profile_text.split("## Official SDKs", maxsplit=1)[1]
    sdk_section = sdk_section.split("## Links", maxsplit=1)[0]

    for spec in ("built in C11", "195 tests", "8.1 MB", "open-source AI coding CLI"):
        assert spec not in sdk_section


def test_profile_refresh_routing_narrative_stays_out_of_sdk_section(profile_text):
    """Partial reverts sometimes duplicate intro routing copy into the SDK section."""
    sdk_section = profile_text.split("## Official SDKs", maxsplit=1)[1]
    sdk_section = sdk_section.split("## Links", maxsplit=1)[0]

    assert "We route your queries through" not in sdk_section
    assert "auto-routes to specialist agents" not in sdk_section
    assert "shader, animation, netcode, level design, QA" not in sdk_section


def test_profile_refresh_newsletter_dispatch_stays_out_of_sdk_section(profile_text):
    sdk_section = profile_text.split("## Official SDKs", maxsplit=1)[1]
    sdk_section = sdk_section.split("## Links", maxsplit=1)[0]
...
def test_profile_refresh_intro_paragraph_matches_canonical_copy(profile_text):
    """4618966 replaced the capability sentence with one canonical routing paragraph."""
    intro = profile_text.split("## What We Build", maxsplit=1)[0]
    paragraph_lines = [
        line.strip()
    ...
    assert paragraph_lines[0] == CANONICAL_INTRO_PARAGRAPH


def test_profile_refresh_monster_gpt_offering_matches_canonical_copy(profile_text):
    """Partial reverts often restore old routing copy while keeping the 145+ intro."""
    offerings = profile_text.split("## What We Build", maxsplit=1)[1]
    offerings = offerings.split("## Official SDKs", maxsplit=1)[0]
    ...
    assert monster_gpt_line == CANONICAL_MONSTER_GPT_OFFERING


def test_profile_refresh_openai_api_offering_matches_canonical_copy(profile_text):
    """4618966 added the OpenAI drop-in claim; partial reverts often truncate the line."""
    offerings = profile_text.split("## What We Build", maxsplit=1)[1]
    offerings = offerings.split("## Official SDKs", maxsplit=1)[0]

    openai_line = next(
        line for line in offerings.splitlines() if line.startswith("- **[OpenAI-compatible API]")
    )
    assert openai_line == CANONICAL_OPENAI_API_OFFERING


def test_profile_refresh_engine_aware_offering_matches_canonical_copy(profile_text):
    """Compile and gameplay-systems claims must stay on the engine-aware offering line."""
    offerings = profile_text.split("## What We Build", maxsplit=1)[1]
    offerings = offerings.split("## Official SDKs", maxsplit=1)[0]

    codegen_line = next(
        line
        for line in offerings.splitlines()
        if line.startswith("- **[Engine-aware code generation]")
    )
    assert codegen_line == CANONICAL_ENGINE_AWARE_OFFERING


def test_profile_refresh_loki_code_offering_matches_canonical_copy(profile_text):
    """4618966 added Loki Code as the fourth offering with blog link and technical specs."""
    offerings = profile_text.split("## What We Build", maxsplit=1)[1]
    offerings = offerings.split("## Official SDKs", maxsplit=1)[0]

    loki_line = next(line for line in offerings.splitlines() if line.startswith("- **[Loki Code]"))
    assert loki_line == CANONICAL_LOKI_CODE_OFFERING


def test_profile_refresh_newsletter_line_matches_canonical_copy(profile_text):
    """4618966 added the newsletter as the fifth link with branded display text."""
    links_section = profile_text.split("## Links", maxsplit=1)[1]
    links_section = links_section.split("A [Luxedeum]", maxsplit=1)[0]
...
def test_profile_refresh_legacy_capability_chain_not_in_intro(profile_text):
    """Partial reverts sometimes restore the pre-asset-pipelines capability list."""
    intro = profile_text.split("## What We Build", maxsplit=1)[0]

    assert PRE_REFRESH_CAPABILITY_CHAIN not in intro


def test_profile_refresh_loki_blog_url_appears_once(profile_text):
    assert profile_text.count(LOKI_CODE_BLOG_URL) == 1


def test_profile_refresh_newsletter_url_appears_once_in_links_only(profile_text):
    links_section = profile_text.split("## Links", maxsplit=1)[1]

    assert profile_text.count(NEWSLETTER_URL) == 1
    assert NEWSLETTER_URL in links_section


def test_profile_refresh_man_in_the_machine_appears_once_in_links_only(profile_text):
    intro = profile_text.split("## What We Build", maxsplit=1)[0]
    offerings = profile_text.split("## What We Build", maxsplit=1)[1]
    offerings = offerings.split("## Official SDKs", maxsplit=1)[0]
    links_section = profile_text.split("## Links", maxsplit=1)[1]

    assert profile_text.count("Man in the Machine") == 1
...
def test_profile_refresh_loki_code_name_appears_once_in_offerings_only(profile_text):
    intro = profile_text.split("## What We Build", maxsplit=1)[0]
    offerings = profile_text.split("## What We Build", maxsplit=1)[1]
    offerings = offerings.split("## Official SDKs", maxsplit=1)[0]
    tail = profile_text.split("## Official SDKs", maxsplit=1)[1]

    assert profile_text.count("[Loki Code]") == 1
    assert "[Loki Code]" in offerings
    assert "[Loki Code]" not in intro
    assert "[Loki Code]" not in tail


def test_profile_refresh_auto_routes_phrase_appears_once_in_offerings_only(profile_text):
