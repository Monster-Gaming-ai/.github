...
def test_validate_workflow_actions_pin_major_version_only():
    """Minor/patch action pins (@v4.2.2) drift silently when upstream releases change."""
    workflow = yaml.safe_load(WORKFLOW_PATH.read_text(encoding="utf-8"))
    action_steps = [
        step["uses"]
        for step in workflow["jobs"]["test"]["steps"]
    ...
    for action_ref in action_steps:
        assert re.fullmatch(r"actions/[\w-]+@v\d+", action_ref), action_ref
        assert "." not in action_ref.split("@", 1)[1]


def test_validate_workflow_setup_python_with_has_only_required_keys():
    """Extra setup-python keys (check-latest, architecture) can change CI behavior unexpectedly."""
    workflow = yaml.safe_load(WORKFLOW_PATH.read_text(encoding="utf-8"))
    setup_python = next(
        step for step in workflow["jobs"]["test"]["steps"]
    ...
    assert set(setup_python["with"].keys()) == {
        "python-version",
        "cache",
