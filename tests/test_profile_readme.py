...
def test_profile_readme_links_section_has_exactly_five_bullets(profile_text):
    """4618966 added newsletter as the fifth link — extra bullets dilute navigation."""
    links_section = profile_text.split("## Links", maxsplit=1)[1]
    links_section = links_section.split("A [Luxedeum]", maxsplit=1)[0]

    link_bullets = [line for line in links_section.splitlines() if line.startswith("- **")]
    assert len(link_bullets) == 5


def test_profile_readme_offerings_section_has_exactly_four_bullets(profile_text):
    """Product offerings must stay a fixed quartet — partial edits drop Loki or Monster-GPT."""
    offerings = profile_text.split("## What We Build", maxsplit=1)[1]
    offerings = offerings.split("## Official SDKs", maxsplit=1)[0]

    offering_bullets = [line for line in offerings.splitlines() if line.startswith("- **[")]
    assert len(offering_bullets) == 4
...
CANONICAL_SDK_TABLE_ROWS = (
    "| TypeScript / JavaScript | [@monstergaming/sdk](https://www.npmjs.com/package/@monstergaming/sdk) | `npm install @monstergaming/sdk` |",
    "| Python | [monstergaming](https://pypi.org/project/monstergaming/) | `pip install monstergaming` |",
    "| Rust | [monstergaming](https://crates.io/crates/monstergaming) | `cargo add monstergaming` |",
)


def test_profile_readme_sdk_table_rows_match_canonical_copy(profile_text):
    """SDK table rows must match exact canonical copy — component checks miss partial row edits."""
    sdk_section = profile_text.split("## Official SDKs", maxsplit=1)[1]
    sdk_section = sdk_section.split("## Links", maxsplit=1)[0]
    ...
    assert data_rows == list(CANONICAL_SDK_TABLE_ROWS)
...
@pytest.mark.parametrize("spec", LOKI_SPEC_SINGLETONS)
def test_profile_readme_loki_technical_specs_appear_once(profile_text, spec):
    """4618966 Loki specs must not be duplicated across sections by partial reverts."""
    assert profile_text.count(spec) == 1, f"Loki spec must appear exactly once: {spec!r}"


@pytest.mark.parametrize("spec", LOKI_SPEC_SINGLETONS)
def test_profile_readme_loki_technical_specs_stay_in_offerings_only(profile_text, spec):
    intro = profile_text.split("## What We Build", maxsplit=1)[0]
    offerings = profile_text.split("## What We Build", maxsplit=1)[1]
    offerings = offerings.split("## Official SDKs", maxsplit=1)[0]
    tail = profile_text.split("## Official SDKs", maxsplit=1)[1]

    assert spec in offerings
    assert spec not in intro
    assert spec not in tail
