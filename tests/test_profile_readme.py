...
@pytest.mark.parametrize("label,url", LINKS_SECTION_PAIRINGS)
def test_profile_readme_links_section_label_url_pairings(profile_text, label, url):
    links_section = profile_text.split("## Links", maxsplit=1)[1]
    links_section = links_section.split("A [Luxedeum]", maxsplit=1)[0]

    expected = f"- **{label}:**"
    matching_lines = [line for line in links_section.splitlines() if line.startswith(expected)]
    assert len(matching_lines) == 1, f"expected exactly one Links entry for {label}"
    assert f"]({url})" in matching_lines[0], f"{label} must link to {url}"


def test_profile_readme_product_offerings_canonical_order(profile_text):
    offerings_section = profile_text.split("## What We Build", maxsplit=1)[1]
    offerings_section = offerings_section.split("## Official SDKs", maxsplit=1)[0]
    ...
    for expected_prefix, line in zip(CANONICAL_OFFERING_ORDER, offering_lines, strict=True):
        assert line.startswith(f"- **{expected_prefix}"), (
            f"offerings must stay in canonical order; expected {expected_prefix}"
        )


def test_profile_readme_monster_gpt_is_flagship_offering(profile_text):
    offerings_section = profile_text.split("## What We Build", maxsplit=1)[1]
    offerings_section = offerings_section.split("## Official SDKs", maxsplit=1)[0]
    ...
    assert "flagship model" in monster_gpt_line


def test_profile_readme_loki_code_test_count_on_offering_line(profile_text):
    loki_line = next(
        line for line in profile_text.splitlines() if "[Loki Code]" in line and line.startswith("-")
    )
