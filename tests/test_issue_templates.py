...
def test_bug_report_fields_follow_triage_order():
    ...
    description_idx = field_ids.index("description")
    reproduction_idx = field_ids.index("reproduction")
    expected_idx = field_ids.index("expected")

    assert description_idx < reproduction_idx, "reporters should describe the bug before steps"
    assert reproduction_idx < expected_idx, "expected behavior should follow reproduction steps"
