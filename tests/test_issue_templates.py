...
def test_bug_report_description_field_has_exact_guidance_text():
    template = _load_template("bug_report.yml")
    description = next(field for field in template["body"] if field["id"] == "description")

    assert description["attributes"]["description"] == "A clear description of what the bug is."


def test_bug_report_reproduction_field_has_exact_guidance_text():
    template = _load_template("bug_report.yml")
    reproduction = next(field for field in template["body"] if field["id"] == "reproduction")

    assert reproduction["attributes"]["description"] == "How can we reproduce the behavior?"
...
def test_feature_request_fields_follow_exact_order():
    """GitHub forms render fields in list order; reordering breaks contributor workflow."""
    template = _load_template("feature_request.yml")
    field_ids = [field["id"] for field in template["body"]]

    assert field_ids == ["problem", "solution", "alternatives"]
...
def test_bug_report_expected_field_label():
    template = _load_template("bug_report.yml")
    expected = next(field for field in template["body"] if field["id"] == "expected")

    assert expected["attributes"]["label"] == "Expected behavior"
    assert expected["attributes"]["description"] == "What did you expect to happen?"


def test_feature_request_problem_field_label():
    template = _load_template("feature_request.yml")
    problem = next(field for field in template["body"] if field["id"] == "problem")

    assert problem["attributes"]["label"] == "Problem"
    assert problem["attributes"]["description"] == "What problem does this feature solve?"


def test_feature_request_alternatives_field_label():
    template = _load_template("feature_request.yml")
    alternatives = next(field for field in template["body"] if field["id"] == "alternatives")
...
@pytest.mark.parametrize("filename", TEMPLATE_FILES.keys())
def test_issue_templates_use_only_input_and_textarea_fields(filename):
    """Markdown/dropdown fields add maintenance cost and can break GitHub form rendering."""
    template = _load_template(filename)
...
@pytest.mark.parametrize("filename", TEMPLATE_FILES.keys())
def test_issue_templates_have_only_expected_top_level_keys(filename):
    """Extra top-level keys (assignees, contact_links) can change triage behavior silently."""
    template = _load_template(filename)

    assert set(template.keys()) == {"name", "description", "labels", "body"}


def test_issue_template_optional_fields_omit_validations_key():
    """Optional fields must omit validations entirely — explicit false differs from absent key."""
    bug_report = _load_template("bug_report.yml")
    bug_fields = {field["id"]: field for field in bug_report["body"]}

    for field_id in ("expected", "version", "environment"):
        assert "validations" not in bug_fields[field_id]

    feature_request = _load_template("feature_request.yml")
    alternatives = next(field for field in feature_request["body"] if field["id"] == "alternatives")

    assert "validations" not in alternatives


def test_issue_template_required_fields_include_validations_block():
    """Required fields must declare validations explicitly — missing block breaks GitHub form enforcement."""
    bug_report = _load_template("bug_report.yml")
    for field_id in ("description", "reproduction"):
        field = next(field for field in bug_report["body"] if field["id"] == field_id)
        assert field.get("validations", {}).get("required") is True

    feature_request = _load_template("feature_request.yml")
    for field_id in ("problem", "solution"):
        field = next(field for field in feature_request["body"] if field["id"] == field_id)
        assert field.get("validations", {}).get("required") is True
